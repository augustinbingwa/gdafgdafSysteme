...
from mod_helpers.hlp_auto_genere import auto_genere_prevue_expertise
...
def get_list_by_criteria(request):
	...
	# Charger la liste
	if query:
		lst = FoncierExpertise.objects.filter(query,etat = True)
		if fexp_status=='2':
			lst = lst.exclude(dossier_expertise__exact='')
	else:
		lst = FoncierExpertise.objects.filter(etat = True)
		if fexp_status=='2':
			lst = lst.exclude(dossier_expertise__exact='')
...
def foncier_expertise_ecriture(request):
	...
	try:
		with transaction.atomic():
			...
			# Si penalite existe
			taux_penalite = obj.has_penalite
			penalite = 0
			if somme_taxe > 0 and taux_penalite > 0:
				penalite = ((somme_taxe + accroissement) * taux_penalite) / 100

			MONTANT_DU = somme_taxe + accroissement + penalite
			if somme_taxe > 0:
				#---------------------------------------------------------------------
				...
				if taux_accroisement > 0:
					ni.libelle += ". Avec un taux d'accroissement de " + str(taux_accroisement) + '%, pour un montant de ' + str(intcomma(int(accroissement))) + 'Bif '
				if taux_penalite > 0:
					ni.libelle += ". Avec un taux de Pénalités forfaitaires pour le premier mois de retard de paiement " + str(taux_penalite) + ' Bif sur un montant de ' + str(penalite)
				if taux_accroisement > 0 or taux_penalite > 0:
					ni.libelle += " sur un montant de " + str(intcomma(int(somme_taxe))) + ' Bif'
				...
				obj.save()
				auto_genere_prevue_expertise(ID)
