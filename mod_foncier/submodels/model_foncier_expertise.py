...
class FoncierExpertise(models.Model):
    ...
    nbr_anne = models.IntegerField(validators=[MaxValueValidator(4),MinValueValidator(1)], default=1)
    ...
    @property
    def has_penalite(self):
        """
        Si la declaration a subit un acroissement
        """
        if self.date_declaration:
            if self.annee == self.date_declaration.year:
                for pena in Penalite.objects.all():
                    date_debut = date(self.date_declaration.year, pena.date_debut.month, pena.date_debut.day)
                    date_fin = date(self.date_declaration.year, pena.date_fin.month, pena.date_fin.day)

                    # Test si date de declaration subit un accroissement
                    if date_debut <= self.date_declaration <= date_fin:
                        return int(pena.taux)
    ...
    user_ecriture = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,related_name='%(class)s_requests_ecriture', null=True)
