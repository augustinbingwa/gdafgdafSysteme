...
from mod_transport.models import VehiculeActivite,ArretVehiculeService
...
@login_required(login_url="login/")
@csrf_exempt
def vehicule_activite_arret(request, pk):
    """
    Modification de l'information de l'activité du véhicule
    """
    ckcf = 0
    ...
    data = dict()
    if request.method == 'POST':
        #enregistrment dans la table ArretVehiculeService
        arretvehicule = ArretVehiculeService()
        arretvehicule.motif = request.POST.get('motif')
        arretvehicule.fichier_formulaire_vehicule = None
        arretvehicule.date_arret = dateTimeNow
        arretvehicule.date_ouverture = obj.date_validate
        arretvehicule.date_arret = dateTimeNow
        arretvehicule.note = None
        arretvehicule.date_note = None
        arretvehicule.reponse_note = None
        arretvehicule.demande_annulation_validation = False
        arretvehicule.date_cancel = None
        arretvehicule.activite_id = pk
        arretvehicule.user_arret_id = request.user.id
        arretvehicule.user_cancel_id = None
        arretvehicule.user_note_id = None
        arretvehicule.date_validate = None
        arretvehicule.date_ouverture = obj.date_debut
        if request.POST.get('arretp'):
            arretvehicule.definitif = False
        if request.POST.get('arrett'):
            arretvehicule.definitif = True
        arretvehicule.user_validate_id = None
        arretvehicule.save()
        #liste paginator
        ...
        return redirect("/transport/vehicule_arret_service/")
        data['html_content_list'] = render_to_string(VehiculeArretServiceTemplate.list, context=context)
        data['url_redirect'] = "/transport/vehicule_arret_service/"
    else:
        ...
        for gtp in getpayement:
            if gtp.taxe_montant > gtp.taxe_montant_paye and gtp.entity == 11:
                note += gtp.reference + '  pour vehucule activite </br>'
                ckcf = 1
            if gtp.taxe_montant > gtp.taxe_montant_paye and gtp.entity == 12:
                note += gtp.reference + '  pour droit stationnement </br>'
                ckcf = 1
            if gtp.taxe_montant > gtp.taxe_montant_paye and gtp.entity == 13:
                note += gtp.reference + '  pour vehucule proprietaire </br>'
                ckcf = 1
            if ckcf == 0:
                context = {'obj': obj, }
