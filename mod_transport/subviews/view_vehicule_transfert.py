...
from mod_transport.subforms.form_vehicule_transfert import *
...
def get_list_by_criteria(request):
    ...
    """
    total = VehiculeTransfert.objects.count
    ...
    v_nom_e = SessionHelpers.init_variables(request, 'v_nom_e')
    v_nom_n = SessionHelpers.init_variables(request, 'v_nom_n')
    ...
    query = SessionHelpers.get_query(query, Q(contribuable_encien__nom__icontains=v_nom_e))
    query = SessionHelpers.get_query(query, Q(contribuable_nouveau__nom__icontains=v_nom_n))
    query = SessionHelpers.get_query(query, Q(user_create__username__icontains=v_user_create))

    # Charger la liste
    # if query:
    #     lst = VehiculeTransfert.objects.filter(query)
    #     if v_status=='2':
    #         lst = lst.exclude(fichier_carterose__exact='')
    # else:
    lst = VehiculeTransfert.objects.all()
        # if v_status=='2':
        #     lst = lst.exclude(fichier_carterose__exact='')
    ...
    if is_date_fr_valid(du) and is_date_fr_valid(au):
        ...
        lst = lst.filter(date_transfert__range=[du, au]).order_by('-date_transfert')
...
def get_context(request):
    ...
    context = {
        ...
        'v_nom_e': request.session['v_nom_e'],
        'v_nom_n': request.session['v_nom_n'],
...
def vehicule_transfert_list(request):
    """
    Liste des véhicules
    """
    ...
    return render(request, VehiculeTransfertTemplate.index, context=get_context(request))

#----------------------------------------------------------------
@login_required(login_url="login/")
def vehicule_transfert_create(request, pk):
    """
    ...
    if request.method == 'POST':
        form = VehiculeTransfertForm(request.POST)
    else:
        form = VehiculeTransfertForm()

    return save_vehicule_form(request, form,obj,etat, VehiculeTransfertTemplate.create, 'createtransf')

#----------------------------------------------------------------
@login_required(login_url="login/")
def vehicule_transfert_update(request, pk):
    """
    Modification de l'information du véhicule
    """
    obj = get_object_or_404(VehiculeTransfert, pk=pk)
    etat = '1'
    if request.method == 'POST':
        form = VehiculeTransfertForm(request.POST, instance=obj)
    else:
        form = VehiculeTransfertForm(instance=obj)
    
    return save_vehicule_form(request, form, obj, etat, VehiculeTransfertTemplate.update, 'update')

#----------------------------------------------------------------
@login_required(login_url="login/")
def vehicule_transfert_delete(request, pk):
    """
    ...
    if request.method == 'POST':
        ...
        data['html_content_list'] = render_to_string(VehiculeTransfertTemplate.list, context=get_context(request))
        data['url_redirect'] = request.session['url_list']
    else:
        context = {'obj': obj}
        data['html_form'] = render_to_string(VehiculeTransfertTemplate.delete, context, request=request)
    
    return JsonResponse(data)

#----------------------------------------------------------------
def save_vehicule_form(request, form,obj, etat, template_name, action):
    """
    ...
    context = {'form': form , 'obj': obj, 'etat': etat}
...
#----------------------------------------------------------------
@csrf_exempt  # pour les methode POST qui necessite crsf_token
def vehicule_transfert_validate(request):
    """
    ...
    ID = request.POST["id"]
    obj = get_object_or_404(VehiculeTransfert, pk=ID)
    ...
    try:
        with transaction.atomic():
            obj.date_transfert = dateTimeNow
            obj.save()
...
#----------------------------------------------------------------
def vehicule_transfert_upload(request, pk):
    """
    Upload du fichier 'CARTE ROSE' du véhicule rémunéré
    """
    obj = get_object_or_404(VehiculeTransfert, pk=pk)
    if request.method == 'POST':
        form = NonRedevabiliteFileUploadForm(request.POST, request.FILES)
        if form.is_valid():
            if form.cleaned_data['fichier_non_redevabilite'] is not None:
                obj.fichier_non_redevabilite = form.cleaned_data['fichier_non_redevabilite']
                obj.save()
            return redirect('vehicule_transfert_list')
        else:
            return ErrorsHelpers.show(request, form)
    else:
        data = dict()
        context = {'obj': obj}
        data['html_form'] = render_to_string(VehiculeTransfertTemplate.upload, context, request=request)

    return JsonResponse(data)

#----------------------------------------------------------------
def vehicule_transfert_upload_cr(request, pk):
    """
    Upload du fichier 'CARTE ROSE' du véhicule rémunéré
    """
    obj = get_object_or_404(VehiculeTransfert, pk=pk)
    if request.method == 'POST':
        ...
        data = dict()
        context = {'obj': obj}
        data['html_form'] = render_to_string('vehicule_transfert/include/_vehicule_transfert_upload_cr.html', context, request=request)

    return JsonResponse(data)

#----------------------------------------------------------------
def vehicule_transfert_upload_temp(request, pk):
    """
    Upload du fichier 'CARTE ROSE' du véhicule rémunéré
    """
    obj = get_object_or_404(Vehicule, pk=pk)
    if request.method == 'POST':
        ...
        data = dict()
        context = {'obj': obj}
        data['html_form'] = render_to_string('vehicule_transfert/include/_vehicule_transfert_upload_temp.html', context, request=request)
